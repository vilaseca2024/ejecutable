    ...
    for ruta in rutas_pdfs:
        ...
        def expandir_rango(cadena):
            ...
            for i in range(len(partes) - 1):
                ...
                match_inicio = re.match(r'([A-Z]+)(\d+)(-\d+)?', inicio)
                match_fin = re.match(r'([A-Z]+)(\d+)(-\d+)?', fin)

                if match_inicio and match_fin:
                    prefijo1, num_inicio, sufijo1 = match_inicio.groups()
                    prefijo2, num_fin, sufijo2 = match_fin.groups()

                    if prefijo1 == prefijo2 and sufijo1 == sufijo2:
                        num_inicio = int(num_inicio)
                        num_fin = int(num_fin)
                        sufijo = sufijo1 if sufijo1 else ''
                        ancho = max(len(str(num_inicio)), len(str(num_fin)))

                        for n in range(num_inicio, num_fin + 1):
                            resultado.append(f"{prefijo1}{str(n).zfill(ancho)}{sufijo}")
                    else:
            ...
            return list(set(resultado))  # eliminar duplicados por seguridad
        ...
        def es_parecido(a, b):
            ...
            # Expandir si hay rangos
            lista_a = expandir_rango(a) if '/' in a or '-' in a else [a]
            lista_b = expandir_rango(b) if '/' in b or '-' in b else [b]

            # Comparar uno a uno
            for item_a in lista_a:
                for item_b in lista_b:
                    if item_a in item_b or item_b in item_a or SequenceMatcher(None, item_a, item_b).ratio() > 0.7:
                        return True
            return False
   
        for row_dict in rows_data:
            cc0 = row_dict.get("J.", "").strip()
            cc1 = row_dict.get("I1.", "").strip()
            cc2 = row_dict.get("A0.", "").strip()
            cc3 = row_dict.get("H10.", "").strip()
            cc4 = row_dict.get("H11.", "").strip()
            matched_row = None
            for row in range(3, ws.max_row + 1):
                val_cc0 = str(ws.cell(row=row, column=headers.index("CC0.") + 1).value or "").strip()
                val_cc1 = str(ws.cell(row=row, column=headers.index("CC1.") + 1).value or "").strip()
                val_cc2 = str(ws.cell(row=row, column=headers.index("CC2.") + 1).value or "").strip()
                val_cc3 = str(ws.cell(row=row, column=headers.index("CC3.") + 1).value or "").strip()
                val_cc4 = str(ws.cell(row=row, column=headers.index("CC4.") + 1).value or "").strip()

                coincidencias = sum([
                    es_parecido(cc0, val_cc0),
                    es_parecido(cc1, val_cc1),
                    es_parecido(cc2, val_cc2),
                    es_parecido(cc3, val_cc3),
                    es_parecido(cc4, val_cc4)
                ])
            ...
            for col_idx, h in enumerate(headers, start=1):
                ...
                if col_idx in formula_cols:
                    ...
                    v = row_dict.get(key, '')
                try:
                    ws.cell(row=matched_row, column=col_idx, value=float(v))
                except:
                    ws.cell(row=matched_row, column=col_idx, value=v)
