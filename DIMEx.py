...
splash_x = (splash.winfo_screenwidth() // 2) - (splash_width // 2)
splash_y = (splash.winfo_screenheight() // 2) - (splash_height // 2)
splash.geometry(f"{splash_width}x{splash_height}+{splash_x}+{splash_y}")
fondo_color = "#ffffff"
splash.configure(bg=fondo_color)

splash_logo = PhotoImage(file=relative_to_assets("logo.png"))
splash_label = tk.Label(splash, image=splash_logo, bg=fondo_color, borderwidth=0, highlightthickness=0)
...
window.iconbitmap(relative_to_assets("logo.ico"))  # Usa tu función normal aquí

# Puedes poner tus widgets aquí
label = tk.Label(window, text="Bienvenido a DIMEx", bg="#202020", fg="white")
label.pack(pady=50)
...
def extraer_campos_pdf(ruta_pdf):
    ...
    while i < len(campos):
        ...
        if re.match(r'^H8\.([0-9]|10|11|12)$', campo_actual['id']):
            # Verificamos si el siguiente es igual → duplicado
            if i + 1 < len(campos) and campos[i + 1]['id'] == campo_actual['id']:
                campo_duplicado_1 = campo_actual
                campo_duplicado_2 = campos[i + 1]
                partes = campo_duplicado_1['id'].split('.')
                numero = partes[1] if len(partes) > 1 and partes[1] != '' else ""
                nuevo_valor = f"{numero} {campo_duplicado_1['titulo']}".strip()

                if campos_corregidos:
                    campos_corregidos[-1]['valor'] = nuevo_valor

                campos_corregidos.append(campo_duplicado_2)
                ultimo_h8_idx_corregido = len(campos_corregidos) - 1
                en_h8 = True
                i += 2  # Saltamos ambos duplicados
            else:
...
def process_all():
    ...
    x = (waiting.winfo_screenwidth() // 2) - (width // 2)
    y = (waiting.winfo_screenheight() // 2) - (height // 2)
    waiting.geometry(f"{width}x{height}+{x}+{y}")
